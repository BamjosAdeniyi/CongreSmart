    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $class->name }}</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Coordinator</p>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-lg font-semibold text-gray-900 dark:text-gray-200">{{ $class->members->count() }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Meeting Time</p>
                        <p class="text-lg font-semibold text-gray-900 dark:text-gray-200">{{ ucfirst($class->meeting_day) }}</p>
                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $class->meeting_time }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-lg font-semibold text-gray-900 dark:text-gray-200">{{ $attendanceStats['average_attendance'] }}</p>
                        <p class="text-sm text-gray-600 dark:text-gray-400">per session</p>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold dark:text-gray-200">Class Members</h2>
                        <a href="{{ route('sabbath-school.assign-members', $class) }}"
                            ...
                            @foreach($class->members as $member)
                                <div class="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 class="font-medium dark:text-gray-200">{{ $member->first_name }} {{ $member->last_name }}</h4>
                                            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $member->phone ?? 'No phone' }}</p>
                                        </div>
                                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold dark:text-gray-200">Recent Attendance</h2>
                        <a href="{{ route('sabbath-school.attendance', $class) }}"
                           class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 text-sm font-medium">
                            Take Attendance
                        </a>
                    ...
                    @if($class->attendance->count() > 0)
                        <div class="space-y-3">
                            ...
                            @foreach($attendanceByDate as $date => $records)
                                ...
                                <div class="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    <div>
                                        <h4 class="font-medium dark:text-gray-200">{{ \Carbon\Carbon::parse($date)->format('M j, Y') }}</h4>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $presentCount }} of {{ $totalMembers }} present</p>
                                    </div>
                                    <div class="text-right">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            @if($totalMembers > 0 && ($presentCount / $totalMembers) >= 0.8) bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300
                                            @elseif($totalMembers > 0 && ($presentCount / $totalMembers) >= 0.6) bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300
        ...
        @if($class->location)
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                <h2 class="text-lg font-semibold mb-4 dark:text-gray-200">Location</h2>
                <div class="flex items-center gap-3">
                    <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                    <span class="text-gray-900 dark:text-gray-200">{{ $class->location }}</span>
                </div>
            </div>
        @endif
    </div>
